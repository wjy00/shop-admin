    ...
    <el-card>
      ...
      <el-form :model="addForm"
        ...
        <el-tabs v-model="activeIndex"
          ...
          <!-- 1.基本信息标签 -->
          <!-- v-model会绑定标签的name，根据点击改变需要激活的索引 -->
...
export default {
  ...
  methods: {
    ...
    beforeTabLeave(activeName, odlActiveName) {
      // 未选中商品分类阻止Tab标签跳转(return false 阻止跳转)
      // activeName：即将进入的标签页的名称
      // odlActiveName：即将离开的标签页的名称
    ...
    addGoods() {
      this.$refs.addFormRef.validate(async (valid) => {
        ...
        // 发送请求前：需对提交的表单进行处理goods_cat,将其转换成字符串
        // this.addForm.goods_cat = this.addForm.goods_cat.join(',')
        // 以上写法不对：级联选择器绑定的对象goods_cat要求是数组对象

        // 解决办法: lodash包中的方法（深拷贝）：cloneDeep(obj)
        // 将this.addForm进行深拷贝
